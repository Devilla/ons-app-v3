...
const ownerData: ReturnedONS['getOwner'] = {
  owner: '0x123',
  ownershipLevel: 'registrar',
}

const wrapperData: ReturnedONS['getWrapperData'] = {
  fuseObj: {
...
describe('getRegistrationStatus', () => {
  describe('2LD .op', () => {
    it('should return short if a name is less than 3 characters', async () => {
      const result = getRegistrationStatus({ name: 'a.op' })
      expect(result).toBe('short')
    })

    it('should return invalid if no values are provided', async () => {
      const result = getRegistrationStatus({ name: 'test.op' })
  ...
  it('should not return short if name is short but is not .op', () => {
    const result = getRegistrationStatus({
      name: 'a.com',
      wrapperData,
      supportedTLD: true,
