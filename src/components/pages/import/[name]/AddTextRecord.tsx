...
import { Button, Dropdown, Helper, Typography } from '@opdomains/thorin'
...
export const AddTextRecord = ({
  ...
  const handleCheck = async () => {
    try {
      ...
      const result = await prover.queryWithProof('TXT', `_ons.${name}`)
      ...
      console.error('_ons check error: ', e)
  ...
  return (
    <Container>
      ...
      <ButtonRow>
        ...
        <Copyable {...{ label: 'Name', value: '_ons' }} />
      </ButtonRow>
      <Spacer $height="2" />
      {address && (
        <Copyable
