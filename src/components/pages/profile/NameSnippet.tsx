...
import { Button, Typography } from '@opdomains/thorin'
...
const NameOwnerItem = ({ address = '', network }: { address?: string; network: number }) => {
  const { getName } = useOns()
  const { data } = useQuery(['getName', address], () => getName(address), {
    enabled: !!address,
  })
  const hasOns = data?.match && data?.name
...
export const NameDetailSnippet = ({
  ...
  ownerData: Exclude<ReturnedONS['getOwner'], undefined>
  wrapperData: Exclude<ReturnedONS['getWrapperData'], undefined>
  network: number
  showButton?: boolean
  dnsOwner?: string
  isCached?: boolean
