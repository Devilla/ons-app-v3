...
import { Input } from '@opdomains/thorin'
...
export const DogFood = (
  ...
  const { getRecords } = useOns()
  const getRecordsMem = useRef(pMemoize(getRecords))

  const inputWatch = watch('dogfoodRaw')

  const { data: opNameAddress } = useQuery(
    ['opNameAddress', inputWatch],
    async () => {
      try {
  ...
  const finalValue = inputWatch?.includes('.') ? opNameAddress : inputWatch
  useEffect(() => { 
            ...
            hasAddressRecord: async (value) => {
              if(value?.includes('.')) {
                try {
                  ...
                  return result?.address ? undefined : 'ONS Name has no address record'
                } catch (e) {
                  return 'ONS Name has no address record'
                }
                }
              },
            ...validations
