...
import { parseInputType, validateName } from '@opdomains/onsjs/utils/validation'
...
import { BackdropSurface, Portal, Typography, mq } from '@opdomains/thorin'
...
export const SearchInput = ({
    ...
    if (history.length > 0) {
      ...
      return history.filter(
        (item) =>
          ...
          (searchItem.type === 'nameWithDotOp' ? item.value !== `${normalisedName}.op` : true),
      )
    }
    return []
  ...
  const searchItems: AnyItem[] = useMemo(() => {
    ...
    const _searchItems: SearchItem[] =
      _searchItem.type === 'nameWithDotOp'
  ...
  const handleSearch = useCallback(() => {
    ...
    if (selectedItem.type === 'nameWithDotOp') {
      selectedItem = {
        type: 'name',
        value: `${normalisedName}.op`,
      }
    }
    ...
    if (selectedItem.type === 'name') {
      ...
      const isDotOP = labels.length === 2 && labels[1] === 'op'
      if (isDotOP && labels[0].length < 3) {
        return
      }
    }
    let path =
    ...
    if (selectedItem.type === 'nameWithDotOp' || selectedItem.type === 'name') {
