...
describe('SelectPrimaryName', () => {
  ...
  it('should show names', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('test2.op')).toBeInTheDocument()
      expect(screen.getByText('test3.op')).toBeInTheDocument()
    })
  })
  it('should only enable next button if name selected', async () => {
    ...
    fireEvent.click(screen.getByText('test.op'))
    expect(screen.getByTestId('primary-next')).not.toBeDisabled()
  })
  it('should filter out existing primary name for selection', async () => {
    mockRequestWithNames()
    renderHelper({ existingPrimary: 'test.op' })
    await waitFor(() => {
      expect(screen.queryByText('test.op')).not.toBeInTheDocument()
      expect(screen.getByText('test2.op')).toBeInTheDocument()
      expect(screen.getByText('test3.op')).toBeInTheDocument()
    })
  })
  it('should truncate encoded names', async () => {
    mockRequest.mockResolvedValue({
      domains: [
        {
          name: '[2fcba40a1a605acf57a88f10820dd7f474036e9c73660ce1bafdbb9004b92ded].op',
          id: '0x0',
        },
      ],
    })
    renderHelper({})
    await waitFor(() => {
      expect(screen.getByText('[2fc...ded].op')).toBeInTheDocument()
    })
  })
})
