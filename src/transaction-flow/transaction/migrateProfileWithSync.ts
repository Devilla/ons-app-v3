...
import type { RecordOptions } from '@opdomains/onsjs/utils/recordHelpers'
import type { JsonRpcSigner } from '@ethersproject/providers'
import type { TFunction } from 'react-i18next'

import { Profile, PublicONS, RecordItem, Transaction, TransactionDisplayItem } from '@app/types'
...
const transaction = async (signer: JsonRpcSigner, ons: PublicONS, data: Data) => {
  ...
  const profile = await ons.getProfile(data.name)
  ...
  if (profile.resolverAddress !== resolverAddress) {
    resolverProfile = await ons.getProfile(data.name, {
      resolverAddress,
    })
  }
