...
import { HelperProps, PublicONS, Transaction, TransactionDisplayItem } from '@app/types'
...
const displayItems = (
  ...
  },
  {
    ...
    value: t('transaction.extendNames.costValue', {
      ns: 'transactionFlow',
      value: makeDisplay(rentPrice, 5, 'eth'), // name extend cost symbol
    }),
  },
]
...
const transaction = async (signer: JsonRpcSigner, ons: PublicONS, data: Data) => {
  ...
  const labels = names.map((name) => {
    ...
    if (parts[1] !== 'op') throw new Error('Currently only supports .op names')
    return parts[0]
  })

  const price = await ons.getPrice(labels, duration, true)
  const priceWithBuffer = price?.base.mul(110).div(100)

  if (!priceWithBuffer) throw new Error('No price found')
  return ons.renewNames.populateTransaction(names, {
    duration,
